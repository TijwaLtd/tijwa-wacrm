...
The public API lets you drive your tijwa-crm instance from your own
...
3. Copy the key. **The full key is shown exactly once.** tijwa-crm
...
> deploy (the common case for a deployment) is fine as-is. If
...
Rather than polling, register an endpoint and tijwa-crm will POST to it when
...
- `POST /api/v1/webhooks` — register `{ "url": "https://…", "events": ["message.received"] }`. `url` must be `https://`. **The response includes `secret` exactly once** — store it to verify signatures; tijwa-crm keeps only an encrypted copy.
...
curl -X POST https://your-crm.example.com/api/v1/webhooks \
  -H "Authorization: Bearer wacrm_live_xxx" \
  -H "Content-Type: application/json" \
  -d '{ "url": "https://example.com/hooks/tijwa-crm", "events": ["message.received"] }'
...
covers messages tijwa-crm stores (inbox + API sends), not broadcast-only
...
[#245](https://github.com/achiando/tijwa-wacrm/issues/245). Future ideas
