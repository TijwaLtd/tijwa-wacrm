...
/** A structured error from the tijwa-crm API envelope (`{ error: { code, message } }`). */
  ...
  private async request<T>(
    ...
    try {
      ...
      throw new WacrmApiError(
        0,
        'network_error',
        `Could not reach tijwa-crm at ${this.baseUrl}: ${(err as Error).message}`,
      );
    }
