#!/usr/bin/env node
// ============================================================
// tijwa-crm MCP server — entry point.
//
// A stdio Model Context Protocol server that exposes the tijwa-crm
// public API (`/api/v1`) as MCP tools, so an MCP client (Claude
// Desktop, Cursor, etc.) can drive a WhatsApp CRM in
// natural language.
//
...
async function main(): Promise<void> {
  ...
  const server = new McpServer({ name: 'tijwa-crm-mcp', version: VERSION });
  ...
  console.error(
    `tijwa-crm MCP server v${VERSION} ready — instance ${config.baseUrl}, ` +
...
main().catch((err) => {
  console.error(`Failed to start tijwa-crm MCP server: ${(err as Error).message}`);
  process.exit(1);
});
