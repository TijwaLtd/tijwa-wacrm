...
-- Credit rule: 20% of price_kes converted to credits (floor)
-- ai_conversations derived from credits: credits * 5 (at simple tier: 1 credit = 5 replies)
-- Starter has ai_credit_check_exempt=true (first package, no credit check applied)
...
RETURNS JSONB AS $$
  SELECT CASE p_plan
    ...
    WHEN 'growth' THEN jsonb_build_object(
      ...
      'ai_credits_per_month', floor(10000 * 0.20)::int,
      'ai_credit_check_exempt', false,
      'ai_conversations_per_month', (floor(10000 * 0.20) * 5)::int,
    ...
    WHEN 'enterprise' THEN jsonb_build_object(
      ...
      'ai_credits_per_month', floor(25000 * 0.20)::int,
      'ai_credit_check_exempt', false,
