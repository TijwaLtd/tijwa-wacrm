...
User-visible changes in `tijwa-crm`. Users: when pulling an update,
...
Assistant**; tijwa-crm calls the provider directly with that key, so
there's no per-seat AI fee and your conversation data never leaves
your own infrastructure for a tijwa-crm-run service. The key is stored
...
- **Outbound event webhooks (`/api/v1/webhooks`).** Register an HTTPS
  ...
  **Migration required:** apply
  ...
  ([#245](https://github.com/achiando/tijwa-wacrm/issues/245))

## [0.3.0] — 2026-07-01

Multi-user accounts ship. Every tijwa-crm install is multi-tenant on the
...
deployments keep working: every existing user is backfilled
...
- **Public REST API (`/api/v1`) — groundwork.** A scoped, revocable
  **API key** system so you can drive tijwa-crm from your own scripts and
  ...
  `supabase/migrations/026_api_keys.sql`. ([#245](https://github.com/achiando/tijwa-wacrm/issues/245))
- **Public REST API — data endpoints.** Built on the key auth above,
  ...
  See `docs/public-api.md`. ([#245](https://github.com/achiando/tijwa-wacrm/issues/245))
...
- **Configurable default deal currency.** Each account can now pick
  ...
  [Default currency](https://tijwa-crm.tijwa.com/docs/settings#deals).
- **Members tab in Settings.** The user-facing surface for the
  ...
  [Members docs](https://tijwa-crm.tijwa.com/docs/members).
...
- **`send_media` flow node.** Send an image (PNG / JPEG / WebP), video
  ...
  ([#156](https://github.com/achiando/tijwa-wacrm/pull/156))
...
- **Inbound WhatsApp messages no longer silently disappear** when two
  ...
  layer. Reported in
  [#136](https://github.com/achiando/tijwa-wacrm/issues/136), fixed in
  [#143](https://github.com/achiando/tijwa-wacrm/pull/143).
...
tijwa-crm is intentionally **single-tenant per WhatsApp number**. RLS on
...
- **Module + schema.** New `flows`, `flow_nodes`, `flow_runs`,
  ...
  ([#112](https://github.com/achiando/tijwa-wacrm/pull/112))
- **Runner engine.** `dispatchInboundToFlows` parses every inbound
  ...
  ([#114](https://github.com/achiando/tijwa-wacrm/pull/114))
- **No-code builder UI** at `/flows`. Linear-list editor with
  ...
  ([#115](https://github.com/achiando/tijwa-wacrm/pull/115))
- **Templates + v1.5 node types.** Three starter templates
  ...
  ([#117](https://github.com/achiando/tijwa-wacrm/pull/117))
...
- **5 color themes** (Violet default, Emerald, Cobalt, Amber, Rose)
  ...
  ([#132](https://github.com/achiando/tijwa-wacrm/pull/132))
- **Theme tokenization sweep** — every previously hard-coded
  ...
  not just the chrome.
  ([#133](https://github.com/achiando/tijwa-wacrm/pull/133))
...
- **Atomic `execution_count` increment** via SECURITY DEFINER RPC —
  ...
  ([#124](https://github.com/achiando/tijwa-wacrm/pull/124))
- **Preload all flow_nodes once per dispatch** — one SELECT per
  ...
  ([#125](https://github.com/achiando/tijwa-wacrm/pull/125))
- **Wasted re-read dropped** after reprompt reset; `loadActiveRun`
  ...
  ([#126](https://github.com/achiando/tijwa-wacrm/pull/126))
...
- **PII redacted from `reply_received` event payload** — customer
  ...
  ([#123](https://github.com/achiando/tijwa-wacrm/pull/123))
- **Constant-time cron-secret compare** on `/api/flows/cron`
  ...
  ([#127](https://github.com/achiando/tijwa-wacrm/pull/127))
...
- **`/flows` no longer spuriously redirects to `/dashboard`** when
  ...
  ([#128](https://github.com/achiando/tijwa-wacrm/pull/128))
